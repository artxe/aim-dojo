...
The mode contract, 2D/3D rules, stats flow, scoring and pooling rules load with `docs/js/game_mode/**` (`.claude/rules/game_mode.md`); the allocation rules are in the root `CLAUDE.md`.
...
3. **`docs/js/game_mode/<name>.js`** — default-exports `/** @type {GameMode} */`. Shape: imports, then module-level declarations (UPPER consts, then per-file role groups), then `function`s, then `export default`, then the bare `{ … }` init block LAST.
   ...
   - `init()`: `set_hud_labels(run_score, accuracy, crit_rate)` (the score UNIT, the accuracy label, the third label), `reset_run_state()`, then `update_camera_view()` FIRST (it populates `state.camera.sens`/`fov`/`impact.rad_size`), then force `state.camera.dimension = "2d"` if the mode is 2D-only, then build the first target(s).
   - `on_frame()` / `render()` / `shoot()` / `update_hud()` / `check_stats()` / `dispose()` per the contract; `update_dimension()` only for a 2D/3D-capable mode (its PRESENCE is also what makes `on_mousemove` clamp the 2D `y` to the ±90° pitch range, so a fixed-2D mode must not define one), opening with `if (state.camera.dimension == "2d") { return }` after `update_camera_view()` and converting camera, targets and any in-flight lerp endpoints.
...
## Verify
- `pnpm run lint` clean — it also regenerates the `<link rel="modulepreload">` block for the new module (stale = silently slower boot, not an error).
