...
- `update_camera_view()` ALSO derives `state.camera.sens` (cached `calc_rad_per_px`) and `state.impact.rad_size = 2*tan(fov*PI/360)/width*20` at the end. Every new `sens` branch must keep these derivations working.
...
- `on_resize()` does NOT call `update_camera_view()`. FOVs derived from `convert_deg_across_aspect(...)` go stale after resize until the next dimension flip.
...
## Hot paths
- Start: `controller/menu.js` sets `state.game.mode` → `logic.start_game()` → request lock+fullscreen → `mode.init()` → first rAF
...
- `state.stats` — shared 30s scoring window (`CONSTANTS.stats.window_ms`); modes push to `shoots`; `check_stats()` expires + decrements aggregates
