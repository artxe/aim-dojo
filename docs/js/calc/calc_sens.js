...
let r6_file_width = 0
/** @type {RbFields?} */
let rb_fields_cache = null
let rb_fields_height = 0
let rb_fields_width = 0
/**
 * @param {number} sens
 * @returns {number}
 */
 ...
 * @returns {number}
 */
export function calc_sens_cs2_yaw(width, height) {
	const { hipfire } = constants.fov.cs2
	const { decimals, scale, yaw_decimals } = constants.sens.cs2
	const exact = calc_sens_cs2(hipfire, width, height)
	return round_to(
		scale * exact / round_to(exact, decimals),
		yaw_decimals
	)
}
/**
...
export function calc_sens_r6_file(width, height) {
	...
	const unit_exact = hipfire * yaw_scale
	let min_yaw_err = Number.POSITIVE_INFINITY
	let yaw = 1
	let yaw_unit = unit_exact
	for (let y = 1; y <= slider_max; y++) {
		const unit = round_to(unit_exact / y, decimals)
		const err = abs(unit * y / unit_exact - 1)
		if (err < min_yaw_err - EPS) {
			min_yaw_err = err
			yaw = y
			yaw_unit = unit
		}
 ...
 * @returns {RbFields}
 */
export function calc_sens_rb_fields(width, height) {
	if (
		rb_fields_cache
		&& rb_fields_width == width
		&& rb_fields_height == height
	) {
		return rb_fields_cache
	}
	const { ads_ref, base, rivals } = constants.fov.rb
	const {
		ads_max,
		decimals,
		grid,
	...
	const aim_ref = ads_ref.rifle
	const hipfire = round_to(
		calc_sens_rb(rivals.hipfire, width, height),
		decimals
	)
	const base_exact = calc_sens_rb(base, width, height)
	const aim_exact = calc_sens_rb(aim_ref, width, height)
	const aiming_exact = aim_exact * base * 100 / (base_exact * aim_ref)
	const aiming = clamp(
		field_min,
		round(aiming_exact / grid) * grid,
		ads_max
	)
	const target_ads = base_exact * aiming_exact / aiming
	let effective = 0
	let horizontal = 0
	let min_err = Number.POSITIVE_INFINITY
	let sensitivity = 0
	for (let s = field_min; s <= field_max; s += grid) {
		for (let h = field_min; h <= field_max; h += grid) {
			const e = hipfire * s * h / 10_000
			const err = abs(e / target_ads - 1)
			if (
				err < min_err - EPS
				|| err < min_err + EPS && abs(h - 100) < abs(horizontal - 100)
			) {
				effective = e
				horizontal = h
				min_err = err
	...
	return rb_fields_cache = {
		...
		hipfire,
		horizontal,
		horizontal_ads: target_ads / hipfire * 10_000 / sensitivity,
		horizontal_hipfire: base_exact / hipfire * 10_000 / sensitivity,
		sensitivity
	}
}
/**
...
export function calc_sens_snap(
	...
	if (sens_name == "cs2") {
		...
		const { decimals, mul_decimals, scale } = constants.sens.cs2
		const m_yaw = calc_sens_cs2_yaw(width, height)
		const exact = calc_sens_cs2(hipfire_fov, width, height)
		const base = round_to(exact, decimals) * m_yaw / scale
		if (!ads_fov) {
			return base / exact
		}
		const mul = calc_sens_cs2(ads_fov, width, height) / base
		return round_to(mul, mul_decimals) / mul
	}
	if (sens_name == "fn") {
	...
	if (sens_name == "r6") {
		...
		const { yaw_unit: yaw_scale } = constants.sens.r6
		const hipfire = calc_sens_r6(base, width, height)
		const { ads_unit, yaw, yaw_unit } = calc_sens_r6_file(width, height)
		const snap = yaw * yaw_unit / (hipfire * yaw_scale)
		if (!ads_fov) {
			return snap
		}
	...
	if (sens_name == "rb") {
		const { base } = constants.fov.rb
		const { aiming, effective } = calc_sens_rb_fields(width, height)
		if (!ads_fov) {
			return effective / calc_sens_rb(base, width, height)
		}
		return effective * aiming / 100 * ads_fov / base
			/ calc_sens_rb(ads_fov, width, height)
	}
	if (sens_name == "sa") {
