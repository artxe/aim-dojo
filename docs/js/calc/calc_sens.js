 ...
 * @returns {number}
 */
export function calc_sens_lol(dpi, target_dpi) {
	let best_sens = 0
	let best_error = Infinity
	for (let sens = 0; sens <= 100; sens += 5) {
		const error = abs(
			dpi * lol_sens_to_dpi_scale(sens) - target_dpi
		)
		if (error < best_error) {
			best_error = error
			best_sens = sens
...
export function lol_sens_to_dpi_scale(sens) {
	...
	const t = sens / 10 - i
	return lol_dpi_scale_points[i] + (lol_dpi_scale_points[i + 1] - lol_dpi_scale_points[i]) * t
}