import { convert_deg_across_aspect, PSI } from "./math.js"
export default /** @type {const} */({
	audio: {
		bgm_max_gain: 1,
		bgm_volume: 10,
		sfx_max_gain: 5,
		sfx_volume: 10
	},
	bg: {
		append_ahead_s: 15,
		evict_behind_s: 3,
		marquee_min_chars: 18,
		marquee_s_per_char: .1,
		type: "default",
		youtube_link: "https://www.youtube.com/watch?v=83C3TZ4Zm_o"
	},
	cpi: {
		ratio: PSI,
		ratio_decimals: 5,
		ref_width: 1_920,
		x: 377
	},
	cpi_norm: {
		cpi: 800,
		default_sens: /** @satisfies {Record<GameSensName, number>} */({
			al: 5,
			bdo: 50,
			cs2: 2.5,
			fn: 50,
			lol: 50,
			mc: .5,
			ow: 15,
			pubg: 50,
			r6: 50,
			rb: 1,
			sa: 20,
			val: 1
		}),
		game: "val"
	},
	crosshair: {
		color: "rgba(0,255,0,1)",
		dot: 0,
		gap: 4,
		height: 8,
		thickness: 8,
		width: 8
	},
	crosshair_preview: {
		fill_style: "rgba(255,255,255,.08)",
		line_width: 1,
		stroke_style: "rgba(255,255,255,.18)"
	},
	fitts: { trial_min: 5, we_factor: 4.133 },
	fov: /** @satisfies {FovConstants} */({
		al: (() => {
			const hipfire = 70 * 1.55
			const x1 = 60 * 1.55
			const x10 = convert_deg_across_aspect(x1, 10, 1)
			const x2 = convert_deg_across_aspect(x1, 2, 1)
			const x3 = convert_deg_across_aspect(x1, 3, 1)
			const x4 = convert_deg_across_aspect(x1, 4, 1)
			const x6 = convert_deg_across_aspect(x1, 6, 1)
			const x8 = convert_deg_across_aspect(x1, 8, 1)
			return {
				ads: { x1, x10, x2, x3, x4, x6, x8 },
				axis: "43",
				hipfire,
				x1,
				x10,
				x2,
				x3,
				x4,
				x6,
				x8
			}
		})(),
		bdo: { axis: "v", hipfire: 70, zoom: 40 },
		cs2: (() => {
			const aug = 45
			const auto2 = 15
			const awp2 = 10
			const hipfire = 90
			const x1 = 40
			return {
				ads: { aug, auto: x1, awp: x1 },
				ads_ref: { aug: x1, auto: x1, awp: x1 },
				ads_stage2: { aug: 0, auto: auto2, awp: awp2 },
				aug,
				auto2,
				awp2,
				axis: "43",
				hipfire,
				x1
			}
		})(),
		fn: (() => {
			const hipfire = 80
			const scoped15 = 15
			const scoped40 = 40
			const targeting65 = 65
			return {
				ads: {
					scoped15,
					scoped40,
					targeting65,
					targeting80: hipfire
				},
				axis: "h",
				hipfire,
				scoped15,
				scoped40,
				targeting65
			}
		})(),
		lol: { axis: "v", hipfire: 120 },
		mc: { axis: "v", hipfire: 110 },
		ow: (() => {
			const ashe = 40
			const emre = 42.5
			const freja = 47.5
			const widow = 30
			return {
				ads: { ashe, emre, freja, widow },
				axis: "h",
				hipfire: 103,
				scopes: { ashe, axis: "v", emre, freja, widow }
			}
		})(),
		pubg: (() => {
			const base = 80
			const x1 = base - 5
			const x15 = base / 12
			const x2 = base / 2
			const x3 = base / 3
			const x4 = base / 4 - 1
			const x6 = base / 6
			const x8 = base / 8
			return {
				ads: { x1, x15, x2, x3, x4, x6, x8 },
				axis: "h",
				base,
				fpp: 103,
				x1,
				x15,
				x2,
				x3,
				x4,
				x6,
				x8
			}
		})(),
		r6: (() => {
			const base = 90
			const x1 = 81
			const x12 = convert_deg_across_aspect(base, 12, 1)
			const x1_5 = convert_deg_across_aspect(base, 1.5, 1)
			const x2 = convert_deg_across_aspect(base, 2, 1)
			const x2_5 = convert_deg_across_aspect(base, 2.5, 1)
			const x3 = convert_deg_across_aspect(base, 3, 1)
			const x4 = convert_deg_across_aspect(base, 4, 1)
			const x5 = convert_deg_across_aspect(base, 5, 1)
			return {
				ads: { x1, x12, x1_5, x2, x2_5, x3, x4, x5 },
				axis: "v",
				base,
				x1,
				x12,
				x1_5,
				x2,
				x2_5,
				x3,
				x4,
				x5
			}
		})(),
		rb: (() => {
			const base = 120
			const exogun = 75
			const minigun = 112.5
			const pistol = 60
			const rifle = 45
			const shotgun = 90
			const slingshot = 105
			return {
				ads: {
					exogun,
					minigun,
					pistol,
					rifle,
					shotgun,
					slingshot
				},
				ads_ref: {
					exogun: rifle,
					minigun: rifle,
					pistol: rifle,
					rifle,
					shotgun: rifle,
					slingshot: rifle
				},
				axis: "v",
				base,
				rivals: {
					exogun,
					hipfire: 70,
					minigun,
					pistol,
					rifle,
					shotgun,
					slingshot
				}
			}
		})(),
		sa: (() => {
			const hipfire = 85
			return {
				axis: "43",
				hipfire,
				x15: convert_deg_across_aspect(hipfire, 15, 1),
				x15_sens: .15,
				x5: convert_deg_across_aspect(hipfire, 5, 1),
				x5_sens: .25
			}
		})(),
		val: (() => {
			const hipfire = 103
			const guardian = convert_deg_across_aspect(hipfire, 1.5, 1)
			const marshal = convert_deg_across_aspect(hipfire, 3.5, 1)
			const operator2_5 = convert_deg_across_aspect(hipfire, 2.5, 1)
			const operator5 = convert_deg_across_aspect(hipfire, 5, 1)
			const spectre = convert_deg_across_aspect(hipfire, 1.15, 1)
			const vandal = convert_deg_across_aspect(hipfire, 1.25, 1)
			return {
				ads: {
					guardian,
					marshal,
					operator: operator2_5,
					spectre,
					vandal
				},
				ads_ref: {
					guardian: vandal,
					marshal: operator2_5,
					operator: operator2_5,
					spectre: vandal,
					vandal
				},
				ads_stage2: {
					guardian: 0,
					marshal: 0,
					operator: operator5,
					spectre: 0,
					vandal: 0
				},
				axis: "h",
				guardian,
				hipfire,
				marshal,
				operator2_5,
				operator5,
				spectre,
				vandal
			}
		})()
	}),
	frame_limit: {
		space_hold_interval_ms: 1_000 / 30
	},
	game: {
		ads_scope: {
			al: "x1",
			cs2: "awp",
			fn: "targeting80",
			ow: "widow",
			pubg: "x1",
			r6: "x2_5",
			rb: "rifle",
			val: "operator"
		},
		lol_sens: 50,
		pubg_fov: 80,
		sens: "val"
	},
	grid: {
		major_3d: [ .227, .29, .407, .6 ],
		major_every: 9,
		major_line_width: 2,
		major_stroke_style: "rgba(58,74,104,.6)",
		minor_3d: [ .227, .29, .407, .3 ],
		minor_line_width: 1,
		minor_stroke_style: "rgba(58,74,104,.3)",
		size: 80,
		sky_sphere_major_deg: 45,
		sky_sphere_radius: 100,
		sky_sphere_segments_h: 36,
		sky_sphere_segments_w: 72
	},
	guide: {
		color_3d: [ 1, 1, 1, .2 ],
		delay_ms: 200,
		line_width: 2,
		stroke_alpha: .2,
		stroke_style: "white"
	},
	hud: {
		fps_interval_ms: 500,
		update_interval_ms: 50,
		window_ms: 30_000
	},
	impact: {
		alpha: .9,
		crit_3d: [ 1, 0, 0 ],
		crit_style: "red",
		dot_radius: 2,
		duration_ms: 150,
		fade_factor: .75,
		hit_3d: [ 1, 1, 1 ],
		hit_style: "white",
		line_width: 2,
		miss_3d: [ 0, 0, 0 ],
		miss_style: "black",
		px_size: 20,
		rings: 2,
		spacing: .4
	},
	mode: {
		aim_booster: {
			core_ratio: .2,
			grid_height: 420,
			grid_width: 600,
			inc_target_per_sec: 2,
			lives: 3,
			lives_font: "bold 40px monospace",
			lives_y: 96,
			rewind_s: 20,
			spawn_font: "bold 120px monospace",
			start_target_per_sec: 2,
			target_diameter: 70,
			target_grow_ms: 2_000,
			text_fill_style: "rgba(255,255,255,.15)"
		},
		h_tracking: {
			base_speed: .06,
			blink_dist_max: 8,
			blink_dist_min: 3,
			blink_interval_steps: [ 1_000, 1_500, 2_000, 2_500 ],
			core_angle_deg: 30,
			direction_change_step: .2,
			impact_interval_ms: 100,
			move_change_interval_ms: 500,
			size_change_interval_ms: 2_000,
			size_lerp_ms: 500,
			size_steps: [ 1.5, 2, 3, 5 ],
			speed_lerp_ms: 250,
			speed_scale_max: 1.6,
			speed_scale_min: .4,
			speed_steps: [ .5, .75, 1 ],
			trial_ms: 5_000
		},
		precision: {
			far_dist_mul: 60,
			near_dist_mul: 5,
			pitch_limit: 30,
			spawn_y_scale: .2,
			target_radius_mul_max: 1.2,
			target_radius_mul_min: .5,
			trial_shots: 15
		},
		writing: {
			font: "bold monospace",
			font_ratio: .78,
			line_width: 2,
			offset_x: 10,
			sentences: [
				"Calm mind wins every fight.",
				"Control the moment, win the game.",
				"Discipline shapes destiny.",
				"Fear is a liar, ignore it.",
				"Focus creates reality.",
				"Fortune favors the bold.",
				"Hesitation leads to defeat.",
				"Move with purpose, strike with precision.",
				"One mistake costs everything.",
				"Precision is better than speed.",
				"Pressure reveals true skill.",
				"See clearly, act instantly.",
				"Stay calm, aim true, finish strong.",
				"Think less, react better.",
				"Train hard, win easy."
			],
			stroke_style: "black",
			text_alpha: .5,
			text_fill_style: "white"
		}
	},
	render: {
		circle_segments: 96,
		far: 2_000,
		near: .1
	},
	screen_saver: { delay_ms: 5_000 },
	sens: {
		al: {
			decimals: 2,
			mul_decimals: 2,
			scale: .022
		},
		bdo: {
			decimals: 0,
			offset: .000_125,
			step: .000_023_75
		},
		cs2: {
			decimals: 2,
			mul_decimals: 2,
			scale: .022,
			yaw_decimals: 6
		},
		fn: { decimals: 1, scale: .005_555 },
		mc: {
			base: 1.2,
			decimals: 16,
			offset: .2,
			percent: 200,
			step: .6
		},
		ow: {
			decimals: 2,
			mul_decimals: 2,
			scale: .006_6
		},
		pubg: {
			base_sens: 50,
			base_yaw: .044_440_000_444_4,
			converted_base: .02,
			decimals: 6,
			step: 15.051_5
		},
		r6: {
			decimals: 6,
			scale: .005_73,
			slider_max: 100,
			yaw_unit: .02
		},
		rb: {
			ads_max: 200,
			decimals: 3,
			grid: 5,
			max: 400,
			min: 5,
			mul_decimals: 2,
			scale: .5
		},
		sa: {
			decimals: 0,
			offset: .000_45,
			step: .000_03
		},
		val: {
			decimals: 3,
			mul_decimals: 3,
			scale: .07
		}
	},
	staircase: {
		band_max: .85,
		band_min: .7,
		start_level: .5,
		step: .08,
		step_decay: .7,
		step_min: .015
	},
	target: {
		base_radius: 5,
		core_fill_3d: [ .11, .204, .29 ],
		core_fill_style: "#1c344a",
		fill_3d: [ .22, .35, .47 ],
		fill_style: "#385978",
		line_width: 1,
		stroke_3d: [ 1, .27, .27 ],
		stroke_style: "#f44"
	},
	toast: {
		confirm_ms: 1_500,
		error_ms: 3_000,
		notice_ms: 2_000
	},
	tpp: {
		bdo: 9,
		fn: 3,
		fn_ads: 1,
		mc: 4,
		pubg: 3,
		render_dist_scale: 10
	},
	tutorial: { gap: 12, pad: 8 },
	version: "v1.0.0"
})/**/