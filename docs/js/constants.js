...
export default /** @type {const} */({
	...
	mode: {
		aim_booster: {
			guide: "Shoot targets as they spawn — the rate keeps rising.",
		...
		flick: {
			first_dist_mul: 5,
			guide: "Flick through the chain, nearest target first.",
			num_targets: 8,
			pitch_limit: 30,
			target_radius_mul: 1.5
		},
		flick_180: {
			guide: "Hit the random target, then spin 180° for the next.",
		...
		h_tracking: {
			base_speed: .01,
			guide: "Hold fire and track the target left and right.",
			impact_interval_s: .1,
		...
		timing: {
			...
			cross_time_max: .9,
			cross_time_min: .3,
			guide: "Shoot while the target is up — earlier scores more.",
			size_steps: [ 1, 2, 3 ],
			speed_steps: [ 2, 3, 4 ],
		...
		twitch: {
			guide: "Shoot every target fast — a miss clears them all.",
			height_div: 6,
			target_hide_duration_ms: 500,
		...
		v_tracking: {
			base_speed: .01,
			guide: "Hold fire and track the target up and down.",
			impact_interval_s: .1,
			move_change_interval_ms: 500,
			pitch_limit: 90,
			size_change_interval_ms: 2_000,
		...
		writing: {
			guide: "Hold fire and trace the letters.",
			line_width: 2,
			offset_x: 10,
			sentences: [
