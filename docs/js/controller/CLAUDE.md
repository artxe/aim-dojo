...
Parameter-order quirk: the message uses `{dpi_scale, height, width}` but the calc worker fn signature is `(width, height, dpi_scale)` — the dispatch in `calc_worker.js:onmessage` rewrites the order. Don't trust message-key order.
