import {
	calc_dpi_al,
	calc_dpi_bdo,
	calc_dpi_cs2,
	calc_dpi_fn,
	calc_dpi_lol,
	calc_dpi_mc,
	calc_dpi_ow,
	calc_dpi_pubg,
	calc_dpi_r6,
	calc_dpi_rb,
	calc_dpi_sa,
	calc_dpi_val
} from "../calc/calc_dpi.js"
import {
	dpi_norm_dpi_input,
	dpi_norm_game_select,
	dpi_norm_result_el,
	dpi_norm_sens_input
} from "./dom.js"
import { round } from "../math.js"
import state from "../state.js"
{
	dpi_norm_dpi_input.addEventListener("change", on_change_dpi)
	dpi_norm_game_select.addEventListener("change", on_change_game)
	dpi_norm_sens_input.addEventListener("change", on_change_sens)
}
/**
 * @param {Event} ev
 * @returns {void}
 */
function on_change_dpi(ev) {
	const target = /** @type {HTMLInputElement} */(ev.currentTarget)/**/
	localStorage.setItem("dpi_norm.dpi", target.value)
	state.dpi_norm.dpi = Number(target.value)
	update_dpi_norm_result()
}
/**
 * @returns {void}
 */
function on_change_game() {
	const game = /** @type {GameSensName} */(dpi_norm_game_select.value)/**/
	if (game == state.dpi_norm.game) {
		return
	}
	localStorage.setItem(
		"dpi_norm.game",
		state.dpi_norm.game = game
	)
	if (state.dpi_norm.game == "al") {
		dpi_norm_sens_input.value = "5"
		state.dpi_norm.sens = 5
	} else if (state.dpi_norm.game == "bdo") {
		dpi_norm_sens_input.value = "54"
		state.dpi_norm.sens = 54
	} else if (state.dpi_norm.game == "cs2") {
		dpi_norm_sens_input.value = "2.5"
		state.dpi_norm.sens = 2.5
	} else if (state.dpi_norm.game == "fn") {
		dpi_norm_sens_input.value = "50"
		state.dpi_norm.sens = 50
	} else if (state.dpi_norm.game == "lol") {
		dpi_norm_sens_input.value = String(state.game.lol_sens)
		state.dpi_norm.sens = state.game.lol_sens
	} else if (state.dpi_norm.game == "mc") {
		dpi_norm_sens_input.value = "0.5"
		state.dpi_norm.sens = 0.5
	} else if (state.dpi_norm.game == "ow") {
		dpi_norm_sens_input.value = "15"
		state.dpi_norm.sens = 15
	} else if (state.dpi_norm.game == "pubg") {
		dpi_norm_sens_input.value = "50"
		state.dpi_norm.sens = 50
	} else if (state.dpi_norm.game == "r6") {
		dpi_norm_sens_input.value = "50"
		state.dpi_norm.sens = 50
	} else if (state.dpi_norm.game == "rb") {
		dpi_norm_sens_input.value = "1"
		state.dpi_norm.sens = 1
	} else if (state.dpi_norm.game == "sa") {
		dpi_norm_sens_input.value = "50"
		state.dpi_norm.sens = 50
	} else if (state.dpi_norm.game == "val") {
		dpi_norm_sens_input.value = "1"
		state.dpi_norm.sens = 1
	} else {
		throw Error(game)
	}
	localStorage.setItem(
		"dpi_norm.sens",
		dpi_norm_sens_input.value
	)
	update_dpi_norm_result()
}
/**
 * @param {Event} ev
 * @returns {void}
 */
function on_change_sens(ev) {
	const target = /** @type {HTMLInputElement} */(ev.currentTarget)/**/
	const value = target.value.replace(/\.+$/, "")
	localStorage.setItem("dpi_norm.sens", value)
	state.dpi_norm.sens = Number(value)
	update_dpi_norm_result()
}
/** @returns {void} */
export function update_dpi_norm_result() {
	const { game } = state.dpi_norm
	let dpi
	if (game == "al") {
		dpi = calc_dpi_al()
	} else if (game == "bdo") {
		dpi = calc_dpi_bdo()
	} else if (game == "cs2") {
		dpi = calc_dpi_cs2()
	} else if (game == "fn") {
		dpi = calc_dpi_fn()
	} else if (game == "lol") {
		dpi = calc_dpi_lol()
	} else if (game == "mc") {
		dpi = calc_dpi_mc()
	} else if (game == "ow") {
		dpi = calc_dpi_ow()
	} else if (game == "pubg") {
		dpi = calc_dpi_pubg()
	} else if (game == "r6") {
		dpi = calc_dpi_r6()
	} else if (game == "rb") {
		dpi = calc_dpi_rb()
	} else if (game == "sa") {
		dpi = calc_dpi_sa()
	} else if (game == "val") {
		dpi = calc_dpi_val()
	} else {
		throw Error(game)
	}
	dpi_norm_result_el.textContent = String(round(dpi))
}