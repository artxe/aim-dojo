...
dpi_norm_dpi_input.addEventListener("input", on_input_dpi)
dpi_norm_game_btn.addEventListener("change", on_change_game)
dpi_norm_cs2_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_cs2_sens_input.addEventListener("input", on_input_sens)
dpi_norm_cs2_zoom_input.addEventListener("input", on_input_zoom)
dpi_norm_fn_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_fn_sens_input.addEventListener("input", on_input_sens)
dpi_norm_fn_zoom_input.addEventListener("input", on_input_zoom)
dpi_norm_lol_sens_input.addEventListener("input", on_input_sens)
dpi_norm_mc_sens_input.addEventListener("input", on_input_sens)
dpi_norm_ow_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_ow_sens_input.addEventListener("input", on_input_sens)
dpi_norm_ow_zoom_input.addEventListener("input", on_input_zoom)
dpi_norm_pubg_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_pubg_sens_input.addEventListener("input", on_input_sens)
dpi_norm_sa_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_sa_sens_input.addEventListener("input", on_input_sens)
dpi_norm_val_fov_btn.addEventListener("change", on_change_fov)
dpi_norm_val_sens_input.addEventListener("input", on_input_sens)
dpi_norm_val_zoom_input.addEventListener("input", on_input_zoom)
/**
 * @param {Event} ev
 * @returns {void}
 */
function on_change_fov(ev) {
	...
	if (game == "cs2") {
		...
		const prev_fov = state.dpi_norm.fov
		const fov = /** @type {"ads"|"ar_legacy"|"hipfire"|"sniper_legacy"} */(target.value)/**/
		localStorage.setItem(
			"dpi_norm.fov",
			state.dpi_norm.fov = fov
		)
