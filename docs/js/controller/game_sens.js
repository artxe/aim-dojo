...
function current_dpi() {
	const { x } = state.dpi_norm
	const { width } = state.game
	const dpi_x = x || round(
		constants.dpi.x * width / 1_920
	)
	return {
		x: dpi_x,
		y: round(dpi_x * constants.dpi.psi)
	}
}
/**
 ...
 * @returns {void}
 */
function on_change_dpi_x(ev) {
	const target = /** @type {HTMLInputElement} */(ev.currentTarget)/**/
	localStorage.setItem(
		"dpi_norm.x",
		String(
			state.dpi_norm.x = round(Number(target.value))
		)
	)
	update_dpi_display()
	post_update_game_sens()
