...
async function on_drop_pubg_file(ev) {
	...
	const fov = 80
	const hipfire = calc_sens_pubg(fov, height, width)
	const ads = hipfire
	const x2 = calc_sens_pubg(fov / 2, height, width)
	const x3 = calc_sens_pubg(fov / 3, height, width)
	const x4 = calc_sens_pubg(fov / 4, height, width)
	const x6 = calc_sens_pubg(fov / 6, height, width)
	const x8 = calc_sens_pubg(fov / 8, height, width)
	const x15 = calc_sens_pubg(fov / 15, height, width)
	const fpp_fov = calc_pubg_fpp_fov(hipfire, width, height)
