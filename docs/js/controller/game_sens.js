...
import { EPS, abs, fround, round, round_to } from "../math.js"
...
export function current_cpi() {
	const { ratio, ref_width, x: default_x } = constants.cpi
	const { x } = state.cpi_norm
	const { width } = state.game
	const cpi_x = x || round(default_x * width / ref_width)
	return {
		x: cpi_x,
		y: round(cpi_x * ratio)
	}
...
function update_cpi_display() {
	...
	cpi_y_exact_el.textContent = String(
		round_to(x * constants.cpi.ratio, 2)
	)
}
/** @returns {void} */
function update_game_sens() {
	...
	const bdo = calc_sens_bdo(calc_width, calc_height, cpi_scale)
	const bdo_hipfire = round_to(bdo, sens.bdo.decimals)
	const bdo_hipfire_exact = round_to(bdo, 2)
	...
	const cs2_yaw = calc_sens_cs2_yaw(calc_width, calc_height)
	const cs2_base = cs2_hipfire * cs2_yaw / sens.cs2.scale
	const cs2_45 = round_to(
		calc_sens_cs2(cs2.aug, calc_width, calc_height) / cs2_base,
		sens.cs2.mul_decimals
	)
	const cs2_40 = round_to(
		calc_sens_cs2(cs2.x1, calc_width, calc_height) / cs2_base,
		sens.cs2.mul_decimals
	)
	const cs2_15 = round_to(
		calc_sens_cs2(cs2.auto2, calc_width, calc_height) / cs2_base,
		sens.cs2.mul_decimals
	)
	const cs2_10 = round_to(
		calc_sens_cs2(cs2.awp2, calc_width, calc_height) / cs2_base,
		sens.cs2.mul_decimals
	)
	...
	const {
		aiming: rb_ads,
		effective: rb_effective,
		hipfire: rb_hipfire,
		horizontal: rb_horizontal,
		horizontal_ads: rb_horizontal_ads,
		horizontal_hipfire: rb_horizontal_hipfire,
		sensitivity: rb_rivals
	} = calc_sens_rb_fields(calc_width, calc_height)
	const rb_base_sens = rb_effective / rb.base / 100
	...
	const sa = calc_sens_sa(calc_width, calc_height)
	const sa_hipfire = round_to(sa, sens.sa.decimals)
	const sa_hipfire_exact = round_to(sa, 2)
	...
	set_text_if_changed(cs2_yaw_el, cs2_yaw)
	set_text_if_changed(cs2_pitch_el, cs2_yaw)
	set_text_if_changed(fn_hipfire_el, fn_hipfire)
	set_text_if_changed(fn_y_el, fn_hipfire)
	set_text_if_changed(
		lol_dpi_el,
	...
	set_text_if_changed(r6_vertical_el, r6_yaw)
	...
	set_text_if_changed(rb_horizontal_el, rb_horizontal)
	set_text_if_changed(
		rb_horizontal_exact_el,
		`${round_to(rb_horizontal_hipfire, sens.rb.mul_decimals)} / ${round_to(rb_horizontal_ads, sens.rb.mul_decimals)}`
	)
	set_text_if_changed(rb_vertical_el, rb_horizontal)
...
}
{
	...
	cpi_ratio_el.textContent = String(
		round_to(
			constants.cpi.ratio,
