...
import { resize_2d, update_crosshair } from "../render/renderer_2d.js"
...
import { close_game_sens_list } from "./game_sens.js"
...
function bg_video_option(video) {
	...
	if (video.name.length > marquee_min_chars) {
		const track = document.createElement("span")
		const text = `${video.name}    `
		track.className = `a=bg-marquee_${(video.name.length * marquee_s_per_char).toFixed(2)}s_linear_infinite d=inline-block`
...
/** @returns {void} */
export function close_setting_view() {
...
/** @returns {void} */
function on_change_ads_toggle() {
	localStorage.setItem(
		"game.ads_toggle",
		String(
			state.game.ads_toggle = ads_toggle_input.checked
		)
	)
}
/** @returns {void} */
...
function on_change_lang() {
	const lang = /** @type {LangName} */(lang_select.value)/**/
	localStorage.setItem(
		"lang",
		String(state.lang = lang)
	)
}
/** @returns {Promise<void>} */
...
/** @returns {void} */
function on_input_render_scale() {
	const text = /** @type {Text} */(render_scale_input.nextSibling)/**/
	text.textContent = `${render_scale_input.value}%`
	localStorage.setItem(
		"render.scale",
		render_scale_input.value
	)
	state.render.scale = Number(render_scale_input.value)
	resize_2d()
	resize_3d()
	resize_bg()
}
 ...
 * @returns {void}
 */
export function open_setting_view(section) {
	close_game_sens_list()
	setting_view_el.setAttribute("active", "")
	replace_hash(
		section ? section.id : "setting"
	)
	scroll_setting_hash_target(section)
}
/** @returns {void} */
...
async function set_bg_widgets(spec) {
	...
	try {
		...
		await render_bg_options(
			spec.type == "youtube" ? "" : spec.video_id
		)
		bg_type_input.value = spec.type == "default" ? "video" : spec.type
	} finally {
		bg_suppress = false
	}
}
...
/** @returns {void} */
function update_crosshair_widgets() {
	const {
		color,
		dot,
		gap,
	...
	const defaults = constants.crosshair
	reset_crosshair_btn.disabled = color == defaults.color
		&& dot == defaults.dot
		&& gap == defaults.gap
...
}
{
	...
	ads_toggle_input.addEventListener("change", on_change_ads_toggle)
	...
	clear_aim_booster_btn.addEventListener(
		"click",
		game_mode.aim_booster.clear_score
	)
	clear_h_tracking_btn.addEventListener(
		"click",
		game_mode.h_tracking.clear_score
	)
	clear_precision_btn.addEventListener(
		"click",
		game_mode.precision.clear_score
	)
	clear_writing_btn.addEventListener(
		"click",
		game_mode.writing.clear_score
	)
	...
	lang_select.addEventListener("change", on_change_lang)
	modal_backdrop_btn.addEventListener("click", on_click_modal_backdrop)
	render_scale_input.addEventListener("input", on_input_render_scale)
