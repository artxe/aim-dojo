...
let setting_mouse_moved = false
{
	...
	bg_video_options_el.addEventListener(
		"click",
		on_click_bg_video_options
	)
	bg_video_options_el.addEventListener(
		"mousedown",
		on_mousedown_bg_video_options
	)
	...
	clear_full_tracking_btn.addEventListener(
		"click",
		game_mode.full_tracking.clear_best_score
	)
	clear_timing_btn.addEventListener(
		"click",
		game_mode.timing.clear_best_score
	)
	clear_tracking_btn.addEventListener(
		"click",
		game_mode.tracking.clear_best_score
	)
...
function bg_video_option(video) {
	...
	li.className = "ai=center cs=pointer d=flex g=8 p=5"
	li.setAttribute("data-id", video.id)
	li.setAttribute("data-name", video.name)
	li.setAttribute("value", video.id)
	...
	button.setAttribute("data-delete", video.id)
	button.textContent = "×"
 ...
 * @returns {Promise<void>}
 */
async function on_click_bg_video_options(ev) {
	const target = ev.target
	if (!(target instanceof HTMLElement)) {
		return
	}
	const delete_id = target.getAttribute("data-delete")
	if (delete_id) {
		ev.stopPropagation()
		...
		await render_bg_options(
			bg_video_select_btn.value == delete_id ? "" : bg_video_select_btn.value
		)
		return
	}
	...
	if (li.hasAttribute("data-upload")) {
		open_upload_dialog()
		return
	}
...
function on_input_bgm_volume() {
	const text = /** @type {Text} */(bgm_volume_input.nextSibling)/**/
 ...
 * @returns {void}
 */
function on_mousedown_bg_video_options(ev) {
	if (
		ev.target instanceof HTMLElement
		&& ev.target.hasAttribute("data-delete")
	) {
		ev.preventDefault()
	}
}
...
async function on_upload_file(ev) {
	...
	const before_value = bg_video_select_btn.value
	try {
	...
	if (
		...
		|| bg_video_select_btn.value != before_value
	) {
		await render_bg_options(bg_video_select_btn.value)
		return
	}
...
function open_upload_dialog() {
	bg_video_options_el.removeAttribute("open")
...
async function render_bg_options(select_id) {
	...
	for (const li of bg_video_options_el.querySelectorAll("li[data-id]")) {
		li.remove()
	}
	const upload_li = bg_video_options_el.querySelector("li[data-upload]")
	for (const video of videos) {
		bg_video_options_el.insertBefore(
	...
	const current = select_id
		...
		: null
	if (current) {
		select_bg_video_option(current.id, current.name)
	} else {
		select_bg_video_option("default", "Default")
	}
}
/**
 * @param {string} hash
 ...
 * @returns {void}
 */
function select_bg_video_option(value, name) {
	bg_video_select_btn.setAttribute("value", value)
	bg_video_select_btn.textContent = name
	bg_video_options_el.removeAttribute("open")
}
/**
 * @param {BgSpec} spec
...
function staged_bg() {
	...
	const source = bg_video_select_btn.value
