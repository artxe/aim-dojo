...
import { clamp, max, min, round_to } from "../math.js"
...
const CPI_RATIO = round_to(
	constants.cpi.ratio,
	constants.cpi.ratio_decimals
)
...
Start here to find the mouse CPI that feels comfortable, then fine-tune it by going back and forth between Writing and this drill: that is your one true sensitivity.`,
			ko: `Aim Booster —
...
먼저 이 훈련에서 편한 마우스 CPI를 찾고, 그다음 Writing과 이 훈련을 오가면서 정밀하게 조정하세요. 그 CPI가 당신의 인생 감도입니다.`
		}
	},
	{
		el: precision_btn,
...
Every 1 to 2.5 seconds it also blinks a few target widths sideways, so reacquiring a target that has just teleported is trained alongside the smooth tracking.
...
The score is throughput too, in bits per second: how much of the correction the target's movement and blinks demand you actually deliver, a blink counting the moment you are back on the target.
...
1~2.5초마다 옆으로 표적 크기의 몇 배만큼 블링크(순간이동)도 하기 때문에, 순간이동한 표적을 다시 잡는 것까지 매끄러운 트래킹과 함께 연습합니다.
실제 게임에서 적들의 움직임은 대부분 좌우 축에서 발생하기 때문에 손풀기용으로도 적절한 모드입니다.
표적 속도도 같은 방식으로 조준 유지 시간을 따라가며, 표적을 70~85% 시간 동안 붙잡는 지점에 수렴합니다.
점수는 여기서도 초당 비트 단위의 처리율이며, 표적의 움직임과 블링크가 요구하는 보정 중 실제로 따라간 만큼을 잽니다. 블링크는 표적을 다시 잡는 순간 반영됩니다.
...
A lower sensitivity than the one that peaks in Aim Booster scores far more easily here, so once Aim Booster has found the comfortable one, go back and forth between the two modes to fine-tune it.
...
Writing 모드에서는 Aim Booster 모드에서 최고점이 나오는 감도보다 더 낮은 감도를 쓰는 게 높은 점수를 얻기 훨씬 쉽기 때문에, Aim Booster에서 편한 감도를 찾은 뒤 두 모드를 오가면서 정밀하게 조정하는 걸 추천합니다.
...
The tour walks its nine sections next.`,
			ko: `Setting을 누르면 말 그대로 설정 패널이 열립니다.
이제 9개의 설정 항목을 차례대로 살펴보겠습니다.`
		}
	},
	{
...
브라우저의 자동 재생 정책 때문에 첫 클릭이나 키 입력이 있어야 소리가 재생됩니다.`
	...
	{
		el: setting_graphics_el,
		keys: NO_KEYS,
		text: {
			en: `Graphics —
Render Scale is the resolution the game and the background are drawn at, and the result is stretched back to the screen, so below 100% it turns blurry and above it sharpens.
Lower it when the frame rate drops; the crosshair has a layer of its own and stays sharp at every scale, and the sensitivity does not change either way.`,
			ko: `Graphics —
Render Scale은 게임 화면과 배경을 그리는 해상도입니다. 그린 화면을 다시 모니터 크기로 늘리기 때문에 100% 아래로 내리면 뿌옇게, 위로 올리면 더 선명해집니다.
프레임이 부족할 때 낮추면 됩니다. 크로스헤어는 별도의 레이어에 그려서 어떤 배율에서도 선명하며, 감도도 이 설정의 영향을 받지 않습니다.`
		}
	},
	{
...
Recommend's CPI-X and CPI-Y are the recommended pair, and Y/X Ratio is the exact ratio between them.
CPI-Y is fixed at ${CPI_RATIO} × CPI-X whatever the game or FOV, because most games expose no separate vertical sensitivity, so the mouse carries that correction instead.
Even if your mouse cannot set the two axes separately, set it to CPI-X and enter Y/X Ratio into RawAccel — linked right under it — and it applies that ratio per axis for you.
...
Recommend에 표시되는 CPI-X와 CPI-Y가 추천 CPI이며, Y/X Ratio는 두 값의 정확한 비율입니다.
CPI-Y는 게임이나 시야각에 상관없이 CPI-X의 ${CPI_RATIO}배로 고정됩니다. 대부분의 게임은 세로 감도를 따로 설정할 수 없어서 그 보정을 마우스 CPI가 대신 떠안기 때문입니다.
사용 중인 마우스가 축별 CPI 설정을 지원하지 않더라도, 마우스 CPI를 CPI-X로 맞추고 아래에 링크된 RawAccel 프로그램에 Y/X Ratio 값을 입력하면 축별 감도를 적용할 수 있습니다.
