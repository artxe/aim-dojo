...
import state, { create_queue, impacts_pool } from "../state.js"
...
let peak_score = 0
let start_ms = 0
/** @type {typeof state.stats.shoots} */
const shoots = create_queue()
...
function dispose() {
	...
	const shoot_count = shoots.length
	let cut_to = shoot_count
...
function init() {
	...
	state.stats.count_hit = 0
	const count_shoot = state.stats.count_shoot = shoots.length
	for (let i = 0; i < count_shoot; i++) {
		if (shoots.at(i).h) {
