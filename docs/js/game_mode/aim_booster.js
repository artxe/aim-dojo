...
/** @returns {void} */
function draw_lives() {
	const { lives_font, lives_y } = constants.mode.aim_booster
	context_2d.font = lives_font
	context_2d.fillText(LIFE_TEXT[lives], 0, lives_y)
}
/** @returns {void} */
function init() {
	const {
...
/** @returns {void} */
function render() {
	const { spawn_font, text_fill_style } = constants.mode.aim_booster
	const { height, width, x, y } = state.camera
	context_2d.save()
	...
	context_2d.save()
	context_2d.font = spawn_font
	context_2d.textAlign = "center"
	context_2d.textBaseline = "middle"
	context_2d.fillStyle = text_fill_style
