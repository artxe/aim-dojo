...
function on_frame() {
	...
	if (dimension == "2d") {
		if (targets.length) {
			...
			if (!t.cr) {
				const dx = t.x - x
				if (dx ** 2 + (t.y - y) ** 2 <= t.r * t.r) {
					t.r = base_radius * 1.5
					t.cr = calc_core_radius(t.r, base_radius)
				}
			}
		} else {
			...
			while (i <= num_targets) {
				...
				cx += cos(theta) * dist
				cy += sin(theta) * dist
				const cr = calc_core_radius(base_radius * 1.5, base_radius)
				const r = base_radius * 3
				const ty = cy + base_radius * 1.5 - cr
				targets[num_targets - i++] = { cr: 0, cx, cy, r, x: cx, y: ty }
			}
		}
		...
		if (!t.cr) {
			...
			if (dot(d_cam, d_body) >= cos(t.r)) {
				t.r = base_radius_rad * 1.5
				t.cr = calc_core_radius(t.r, base_radius_rad)
			}
		}
	} else {
		const base_radius_rad = px_to_rad(base_radius)
		...
		while (i <= num_targets) {
			...
			cy += cos(theta) * dist
			cp += sin(theta) * dist
			const cr = calc_core_radius(
				base_radius_rad * 1.5,
				base_radius_rad
			)
			const r = base_radius_rad * 3
			const p = cp - base_radius_rad * 1.5 + cr
			targets_3d[num_targets - i++] = { cp, cr: 0, cy, p, r, y: cy }
		}
	}
}
/** @returns {void} */
