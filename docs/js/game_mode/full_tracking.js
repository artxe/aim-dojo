...
const STORAGE_KEY = "full_tracking#best_score"
...
full_tracking_score_el.textContent = String(best_score)
/** @returns {void} */
...
function clear_best_score() {
	...
	set_text_if_changed(full_tracking_score_el, 0)
	send_toast(
		"Full Tracking score has been reset!",
		1_500
	)
}
/** @returns {void} */
...
function init() {
	...
	next_change_v_move_ms = now_ms + move_change_interval_ms
	next_impact_s = 0
	move.direction = 1
	...
	v_move.direction = 1
	v_move.direction_change_rate = 0
	v_move.speed = base_speed
	update_camera_view()
...
function on_frame() {
	...
	if (key_a || key_e || key_q || key_r || key_w || mb_left) {
		...
		if (dimension == "2d") {
			...
			const v_speed = target.r * v_move.speed
			target.x += speed * dt
			target.y += v_speed * dt
			...
			if (v_speed_lerp.active) {
				...
				target.y >= range_px && v_move.direction > 0
				|| target.y <= -range_px && v_move.direction < 0
			) {
				v_move.direction *= -1
				v_move.direction_change_rate = 0
				v_speed_lerp.active = true
				v_speed_lerp.from = v_move.speed
				v_speed_lerp.start_ms = now_ms
				v_speed_lerp.to = -v_move.speed
				next_change_v_move_ms = now_ms + move_change_interval_ms
			} else if (now_ms >= next_change_v_move_ms) {
				if (random() < (v_move.direction_change_rate += .2)) {
					v_move.direction *= -1
					v_move.direction_change_rate = 0
				}
				...
				v_speed_lerp.from = v_move.speed
				v_speed_lerp.start_ms = now_ms
				v_speed_lerp.to = base_speed * v_speed_steps[index] * v_move.direction
				next_change_v_move_ms = now_ms + move_change_interval_ms
			}
			...
			const v_speed = target_3d.r * v_move.speed
			...
			if (v_speed_lerp.active) {
				...
				target_3d.p >= range_rad && v_move.direction > 0
				|| target_3d.p <= -range_rad && v_move.direction < 0
			) {
				v_move.direction *= -1
				v_move.direction_change_rate = 0
...
function update_hud() {
	...
	if (score > peak_score) {
		...
		if (score > best_score) {
			...
			set_text_if_changed(full_tracking_score_el, score)
		}
	}
	set_attr_if_changed(
		peak_score_el,
