...
function dispose() {
	...
	state.mode.full_tracking.size_lerp.active = false
	state.mode.full_tracking.speed_lerp.active = false
...
function init() {
	...
	const { base_speed } = constants.mode.full_tracking
	const {
		move_change_interval_ms,
		size_change_interval_ms
	} = constants.mode.full_tracking
	const { sens } = state.game
	const { target, target_3d } = state.mode.full_tracking
	const { now_ms } = state.timer
	state.camera.dimension = sens == "lol" ? "2d" : "3d"
	state.mode.full_tracking.next_change_size_ms = now_ms + size_change_interval_ms
	state.mode.full_tracking.next_change_move_ms = now_ms + move_change_interval_ms
	state.mode.full_tracking.next_change_v_move_ms = now_ms + move_change_interval_ms
	state.mode.full_tracking.next_impact_s = 0
	state.mode.full_tracking.move.direction = 1
	state.mode.full_tracking.move.direction_change_rate = 0
	state.mode.full_tracking.move.speed = base_speed
	state.mode.full_tracking.v_move.direction = 1
	state.mode.full_tracking.v_move.direction_change_rate = 0
	state.mode.full_tracking.v_move.speed = base_speed
	state.mode.full_tracking.peak_score = 0
...
function on_frame() {
	...
	const {
		...
		target_3d,
		v_speed_lerp
	} = state.mode.full_tracking
	if (key_a || key_e || key_q || key_r || key_w || mb_left) {
		const dt = now_ms - prev_ms
		if (dimension == "2d") {
			const speed = target.r * state.mode.full_tracking.move.speed
			const v_speed = target.r * state.mode.full_tracking.v_move.speed
			target.x += speed * dt
			target.y += v_speed * dt
			...
			} else if (now_ms >= state.mode.full_tracking.next_change_size_ms) {
			...
			if (speed_lerp.active) {
				...
				if (p == 1) {
					...
					state.mode.full_tracking.move.speed = speed_lerp.to
				} else {
					state.mode.full_tracking.move.speed = lerp(speed_lerp.from, speed_lerp.to, p)
				}
			} else if (now_ms >= state.mode.full_tracking.next_change_move_ms) {
				if (random() < (state.mode.full_tracking.move.direction_change_rate += .2)) {
					state.mode.full_tracking.move.direction *= -1
					state.mode.full_tracking.move.direction_change_rate = 0
				}
				const index = random() * speed_steps.length | 0
				speed_lerp.active = true
				speed_lerp.from = state.mode.full_tracking.move.speed
				speed_lerp.start_ms = now_ms
				speed_lerp.to = base_speed * speed_steps[index] * state.mode.full_tracking.move.direction
				state.mode.full_tracking.next_change_move_ms = now_ms + move_change_interval_ms
			}
			if (v_speed_lerp.active) {
				...
				if (p == 1) {
					...
					state.mode.full_tracking.v_move.speed = v_speed_lerp.to
				} else {
					state.mode.full_tracking.v_move.speed = lerp(
						v_speed_lerp.from,
						v_speed_lerp.to,
						p
					)
				...
				target.y >= range_px && state.mode.full_tracking.v_move.direction > 0
				|| target.y <= -range_px && state.mode.full_tracking.v_move.direction < 0
			) {
				state.mode.full_tracking.v_move.direction *= -1
				state.mode.full_tracking.v_move.direction_change_rate = 0
				v_speed_lerp.active = true
				v_speed_lerp.from = state.mode.full_tracking.v_move.speed
				v_speed_lerp.start_ms = now_ms
				v_speed_lerp.to = -state.mode.full_tracking.v_move.speed
				state.mode.full_tracking.next_change_v_move_ms = now_ms + move_change_interval_ms
			} else if (now_ms >= state.mode.full_tracking.next_change_v_move_ms) {
				if (random() < (state.mode.full_tracking.v_move.direction_change_rate += .2)) {
					state.mode.full_tracking.v_move.direction *= -1
					state.mode.full_tracking.v_move.direction_change_rate = 0
				}
				...
				v_speed_lerp.from = state.mode.full_tracking.v_move.speed
				v_speed_lerp.start_ms = now_ms
				v_speed_lerp.to = base_speed * v_speed_steps[index] * state.mode.full_tracking.v_move.direction
				state.mode.full_tracking.next_change_v_move_ms = now_ms + move_change_interval_ms
			}
			const theta = to_rad(
				state.mode.full_tracking.move.direction * 30 - 90
			)
			...
			const speed = target_3d.r * state.mode.full_tracking.move.speed
			const v_speed = target_3d.r * state.mode.full_tracking.v_move.speed
			...
			if (v_speed_lerp.active) {
				...
				target_3d.p >= range_rad && state.mode.full_tracking.v_move.direction > 0
				|| target_3d.p <= -range_rad && state.mode.full_tracking.v_move.direction < 0
			) {
				state.mode.full_tracking.v_move.direction *= -1
				state.mode.full_tracking.v_move.direction_change_rate = 0
...
/** @returns {void} */
function shoot() {
	const { impact_interval_s } = constants.mode.full_tracking
	const { impacts, impacts_3d } = state
	const { dimension, pitch, x, y, yaw } = state.camera
	const { px_size, rad_size } = state.impact
	const { next_impact_s, target, target_3d } = state.mode.full_tracking
	const { shoots } = state.stats
	...
	if (dimension == "2d") {
		...
		if (is_hit && now_s >= next_impact_s) {
			...
			state.mode.full_tracking.next_impact_s = now_s + impact_interval_s
		}
	} else {
...
function update_fov() {
	...
	const { target, target_3d } = state.mode.full_tracking
	const { active } = state.mode.full_tracking.size_lerp
	if (sens == "lol" && !mb_right && dimension == "3d") {
		camera_to_2d()
		state.mode.full_tracking.target = target_to_2d(target_3d)
		if (active) {
			state.mode.full_tracking.size_lerp.from = rad_to_px(
				state.mode.full_tracking.size_lerp.from
			)
			state.mode.full_tracking.size_lerp.to = rad_to_px(
				state.mode.full_tracking.size_lerp.to
			)
		}
	} else if ((sens != "lol" || mb_right) && dimension == "2d") {
		camera_to_3d()
		state.mode.full_tracking.target_3d = target_to_3d(target)
		if (active) {
			state.mode.full_tracking.size_lerp.from = px_to_rad(
				state.mode.full_tracking.size_lerp.from
			)
			state.mode.full_tracking.size_lerp.to = px_to_rad(
				state.mode.full_tracking.size_lerp.to
			)
		}
	}
...
function update_hud() {
	...
	if (score > state.mode.full_tracking.peak_score) {
		state.mode.full_tracking.peak_score = score
		if (score > state.mode.full_tracking.best_score) {
			state.mode.full_tracking.best_score = score
			localStorage.setItem(
				"full_tracking.best_score",
				String(score)
			)
			set_text_if_changed(
				full_tracking_score_el,
	...
	peak_score_el.setAttribute(
		"value",
		`${score} / ${state.mode.full_tracking.peak_score}`
	)
