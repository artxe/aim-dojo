import constants from "../constants.js"
import {
	accuracy_el,
	clear_h_tracking_btn,
	crit_rate_el,
	h_tracking_score_el,
	level_el,
	run_score_el,
	reset_hud_labels,
	send_toast,
	set_attr_if_changed,
	set_hud_labels,
	set_mode_score
} from "../controller/dom.js"
import { lang_text } from "../i18n.js"
import {
	reset_run_state,
	update_camera_view
} from "../logic.js"
import {
	abs,
	calc_core_radius,
	clamp,
	cos,
	lerp,
	log2,
	min,
	random,
	round,
	round_to,
	sin,
	to_rad
} from "../math.js"
import {
	convert_camera_to_2d,
	convert_camera_to_3d,
	convert_target_to_2d,
	convert_target_to_3d,
	hit_test_2d,
	hit_test_3d,
	is_target_visible_2d,
	is_target_visible_3d,
	px_to_rad,
	rad_to_px
} from "../render/camera.js"
import {
	context_2d,
	draw_aim_guides_2d,
	draw_crosshair,
	draw_grid,
	draw_impacts,
	draw_target
} from "../render/renderer_2d.js"
import {
	draw_aim_guides_3d,
	draw_grid_3d,
	draw_impacts_3d,
	draw_target_3d,
	prepare_3d_view,
	record_shot
} from "../render/renderer_3d.js"
import state, { shoots_pool } from "../state.js"
import { create_record } from "../struct/record.js"
import { create_staircase } from "../struct/staircase.js"
const LEVEL_KEY = "h_tracking#level"
const STORAGE_KEY = "h_tracking#runs"
let blink_bits = 0
let guide_lost_ms = 0
let guide_shown = false
let level_dirty = false
const move = {
	core_angle: 0,
	core_from: 0,
	core_to: 0,
	/** @type {1|-1} */
	direction: 1,
	direction_change_rate: 0,
	speed: 0
}
let next_blink_ms = 0
let next_change_move_ms = 0
let next_change_size_ms = 0
let next_impact_ms = 0
const record = create_record(
	localStorage.getItem(STORAGE_KEY) || ""
)
let run_count = 0
let run_sum = 0
const size_lerp = {
	active: false,
	from: 0,
	start_ms: 0,
	to: 0
}
const speed_lerp = {
	active: false,
	from: 0,
	start_ms: 0,
	to: 0
}
const staircase = create_staircase(
	Number(
		localStorage.getItem(LEVEL_KEY) || constants.staircase.start_level
	)
)
/** @type {Target} */
const target = { cr: 0, cx: 0, cy: 0, r: 0, x: 0, y: 0 }
/** @type {Target3D} */
const target_3d = { cp: 0, cr: 0, cy: 0, p: 0, r: 0, y: 0 }
let track_bits = 0
let trial_hit_ms = 0
let trial_shoot_ms = 0
/** @type {Target[]} */
const aim_chain_2d = [ target ]
/** @type {Target3D[]} */
const aim_chain_3d = [ target_3d ]
set_mode_score(
	h_tracking_score_el,
	clear_h_tracking_btn,
	record.value
)
/** @returns {void} */
function blink() {
	const { blink_dist_max, blink_dist_min } = constants.mode.h_tracking
	const { dimension, pitch, x, y, yaw } = state.camera
	const amp = lerp(
		blink_dist_min,
		blink_dist_max,
		random()
	)
	const t = dimension == "2d" ? target : target_3d
	const dist = 2 * t.r * amp * (random() < .5 ? -1 : 1)
	if (dimension == "2d") {
		target.x += dist
		if (!is_target_visible_2d(target, x, y)) {
			target.x -= 2 * dist
		}
	} else {
		target_3d.y += dist
		if (!is_target_visible_3d(target_3d, yaw, pitch)) {
			target_3d.y -= 2 * dist
		}
	}
	blink_bits = log2(amp + 1)
	schedule_blink()
}
/** @returns {void} */
function check_stats() {
	const { shoots } = state.stats
	const { now_ms } = state.timer
	const window_ms = now_ms - constants.hud.window_ms
	while (shoots.length) {
		const first = shoots.at()
		const { c, d, e, h, s } = first
		if (window_ms >= e) {
			const ms = e - s
			state.stats.count_shoot--
			state.stats.sum_shoot_ms -= ms
			track_bits -= d
			if (h) {
				state.stats.count_hit--
				state.stats.sum_hit_ms -= ms
				if (c) {
					state.stats.count_crit--
					state.stats.sum_crit_ms -= ms
				}
			}
			shoots.drop()
			shoots_pool.recycle(first)
		} else if (window_ms >= s) {
			const ms = window_ms - s
			const cut = d && d * ms / (e - s)
			state.stats.sum_shoot_ms -= ms
			track_bits -= cut
			first.d = d - cut
			if (h) {
				state.stats.sum_hit_ms -= ms
				if (c) {
					state.stats.sum_crit_ms -= ms
				}
			}
			first.s = window_ms
			break
		} else {
			break
		}
	}
}
/** @returns {void} */
function clear_score() {
	record.clear()
	localStorage.removeItem(STORAGE_KEY)
	set_mode_score(
		h_tracking_score_el,
		clear_h_tracking_btn,
		0
	)
	send_toast(
		lang_text(
			{
				en: "H-Tracking score has been reset!",
				ko: "H-Tracking 점수를 초기화했습니다!"
			}
		),
		constants.toast.confirm_ms
	)
}
/** @returns {void} */
function dispose() {
	state.game.mode = null
	reset_hud_labels()
	blink_bits = 0
	guide_lost_ms = 0
	guide_shown = false
	size_lerp.active = false
	speed_lerp.active = false
	track_bits = 0
	trial_hit_ms = 0
	trial_shoot_ms = 0
	if (run_count) {
		record.push(run_sum / run_count)
		localStorage.setItem(
			STORAGE_KEY,
			String(record.value)
		)
		set_mode_score(
			h_tracking_score_el,
			clear_h_tracking_btn,
			record.value
		)
	}
	run_count = 0
	run_sum = 0
	if (level_dirty) {
		level_dirty = false
		localStorage.setItem(
			LEVEL_KEY,
			String(staircase.level)
		)
	}
	reset_run_state()
	accuracy_el.removeAttribute("value")
	crit_rate_el.removeAttribute("value")
	level_el.removeAttribute("value")
	run_score_el.removeAttribute("value")
}
/** @returns {void} */
function init() {
	const { base_radius: r } = constants.target
	const {
		base_speed,
		core_angle_deg,
		move_change_interval_ms,
		size_change_interval_ms
	} = constants.mode.h_tracking
	const { now_ms } = state.timer
	set_hud_labels("Bit/s", "On Target", "Critical")
	reset_run_state()
	next_change_size_ms = now_ms + size_change_interval_ms
	next_change_move_ms = now_ms + move_change_interval_ms
	next_impact_ms = 0
	schedule_blink()
	blink_bits = 0
	move.direction = 1
	move.direction_change_rate = 0
	move.speed = base_speed * speed_scale()
	move.core_angle = to_rad(
		move.direction * core_angle_deg - 90
	)
	update_camera_view()
	if (state.camera.dimension == "2d") {
		const cr = calc_core_radius(r)
		const core_offset = r - cr
		target.cr = cr
		target.cx = core_offset * cos(move.core_angle)
		target.cy = core_offset * sin(move.core_angle)
		target.r = r
		target.x = 0
		target.y = 0
	} else {
		const radius_rad = px_to_rad(r)
		const cr = calc_core_radius(radius_rad)
		const alpha = radius_rad - cr
		target_3d.cp = -alpha * sin(move.core_angle)
		target_3d.cr = cr
		target_3d.cy = alpha * cos(move.core_angle)
		target_3d.p = 0
		target_3d.r = radius_rad
		target_3d.y = 0
	}
}
/** @returns {void} */
function on_frame() {
	const { base_radius } = constants.target
	const {
		base_speed,
		core_angle_deg,
		direction_change_step,
		move_change_interval_ms,
		size_change_interval_ms,
		size_lerp_ms,
		size_steps,
		speed_lerp_ms,
		speed_steps
	} = constants.mode.h_tracking
	const { dimension } = state.camera
	const {
		key_a,
		key_e,
		key_q,
		key_r,
		key_w,
		mb_left
	} = state.input
	const { now_ms, prev_ms } = state.timer
	if (key_a || key_e || key_q || key_r || key_w || mb_left) {
		const base = dimension == "2d" ? base_radius : px_to_rad(base_radius)
		const dt = now_ms - prev_ms
		const t = dimension == "2d" ? target : target_3d
		if (dimension == "2d") {
			target.x += base * move.speed * dt
		} else {
			target_3d.y += base * move.speed * dt
		}
		if (size_lerp.active) {
			const p = clamp(
				0,
				(now_ms - size_lerp.start_ms) / size_lerp_ms,
				1
			)
			if (p == 1) {
				size_lerp.active = false
				t.r = size_lerp.to
			} else {
				t.r = lerp(size_lerp.from, size_lerp.to, p)
			}
			t.cr = calc_core_radius(t.r, base)
		} else if (now_ms >= next_change_size_ms) {
			const index = random() * size_steps.length | 0
			size_lerp.active = true
			size_lerp.from = t.r
			size_lerp.start_ms = now_ms
			size_lerp.to = base * size_steps[index]
			next_change_size_ms = now_ms + size_change_interval_ms
		}
		if (speed_lerp.active) {
			const p = clamp(
				0,
				(now_ms - speed_lerp.start_ms) / speed_lerp_ms,
				1
			)
			if (p == 1) {
				speed_lerp.active = false
				move.core_angle = move.core_to
				move.speed = speed_lerp.to
			} else {
				move.core_angle = lerp(move.core_from, move.core_to, p)
				move.speed = lerp(speed_lerp.from, speed_lerp.to, p)
			}
		} else if (now_ms >= next_change_move_ms) {
			if (random() < (move.direction_change_rate += direction_change_step)) {
				move.direction *= -1
				move.direction_change_rate = 0
			}
			const index = random() * speed_steps.length | 0
			move.core_from = move.core_angle
			move.core_to = to_rad(
				move.direction * core_angle_deg - 90
			)
			speed_lerp.active = true
			speed_lerp.from = move.speed
			speed_lerp.start_ms = now_ms
			speed_lerp.to = base_speed * speed_steps[index] * move.direction * speed_scale()
			next_change_move_ms = now_ms + move_change_interval_ms
		}
		if (now_ms >= next_blink_ms) {
			blink()
		}
		const core_offset = t.r - t.cr
		if (dimension == "2d") {
			target.cx = target.x + core_offset * cos(move.core_angle)
			target.cy = target.y + core_offset * sin(move.core_angle)
		} else {
			target_3d.cp = target_3d.p - core_offset * sin(move.core_angle)
			target_3d.cy = target_3d.y + core_offset * cos(move.core_angle)
		}
		shoot()
	}
	update_aim_guide()
}
/** @returns {void} */
function render() {
	const { dimension, height, width, x, y } = state.camera
	context_2d.save()
	context_2d.clearRect(0, 0, width, height)
	context_2d.translate(
		round(width / 2),
		round(height / 2)
	)
	if (dimension == "2d") {
		context_2d.save()
		context_2d.translate(-x, -y)
		draw_grid()
		if (guide_shown) {
			draw_aim_guides_2d(aim_chain_2d)
		}
		draw_target(target, 1)
		draw_impacts()
		context_2d.restore()
	} else {
		prepare_3d_view()
		draw_grid_3d()
		if (guide_shown) {
			draw_aim_guides_3d(aim_chain_3d)
		}
		draw_target_3d(target_3d, 1)
		draw_impacts_3d()
	}
	draw_crosshair()
	context_2d.restore()
}
/** @returns {void} */
function schedule_blink() {
	const { blink_interval_steps } = constants.mode.h_tracking
	const index = random() * blink_interval_steps.length | 0
	next_blink_ms = state.timer.now_ms + blink_interval_steps[index]
}
/** @returns {void} */
function shoot() {
	const {
		impact_interval_ms,
		move_change_interval_ms,
		trial_ms
	} = constants.mode.h_tracking
	const { base_radius } = constants.target
	const { dimension, pitch, x, y, yaw } = state.camera
	const { shoots } = state.stats
	const { now_ms, prev_ms } = state.timer
	const ms = now_ms - prev_ms
	const { is_crit, is_hit } = dimension == "2d"
		? hit_test_2d(target, x, y)
		: hit_test_3d(target_3d, yaw, pitch)
	if (is_hit && now_ms >= next_impact_ms) {
		record_shot(true, is_crit)
		next_impact_ms = now_ms + impact_interval_ms
	}
	const shoot_entry = shoots_pool.obtain()
	shoot_entry.c = is_crit
	const size_mul = dimension == "2d"
		? target.r / base_radius
		: target_3d.r / px_to_rad(base_radius)
	let d = 0
	if (is_hit) {
		d = log2(
			abs(move.speed) * move_change_interval_ms / (2 * size_mul) + 1
		) / move_change_interval_ms * ms
		if (blink_bits) {
			d += blink_bits
			blink_bits = 0
		}
	}
	shoot_entry.d = d
	shoot_entry.e = now_ms
	shoot_entry.h = is_hit
	shoot_entry.s = prev_ms
	shoots.push(shoot_entry)
	state.stats.count_shoot++
	state.stats.sum_shoot_ms += ms
	track_bits += shoot_entry.d
	trial_shoot_ms += ms
	if (is_hit) {
		state.stats.count_hit++
		state.stats.sum_hit_ms += ms
		trial_hit_ms += ms
		if (is_crit) {
			state.stats.count_crit++
			state.stats.sum_crit_ms += ms
		}
	}
	if (trial_shoot_ms >= trial_ms) {
		staircase.feed(trial_hit_ms / trial_shoot_ms)
		level_dirty = true
		trial_hit_ms = 0
		trial_shoot_ms = 0
	}
}
/** @returns {number} */
function speed_scale() {
	const { speed_scale_max, speed_scale_min } = constants.mode.h_tracking
	return lerp(
		speed_scale_min,
		speed_scale_max,
		staircase.level
	)
}
/** @returns {void} */
function update_aim_guide() {
	const { delay_ms } = constants.guide
	const { dimension, pitch, x, y, yaw } = state.camera
	const { now_ms } = state.timer
	if (
		dimension == "2d"
			? is_target_visible_2d(target, x, y)
			: is_target_visible_3d(target_3d, yaw, pitch)
	) {
		guide_lost_ms = 0
		guide_shown = false
		return
	}
	if (!guide_lost_ms) {
		guide_lost_ms = now_ms
	}
	guide_shown = now_ms - guide_lost_ms >= delay_ms
}
/** @returns {void} */
function update_dimension() {
	const { dimension, pitch, x, y, yaw } = state.camera
	const { active } = size_lerp
	update_camera_view()
	if (dimension == "2d") {
		if (state.camera.dimension == "2d") {
			return
		}
		if (x || y) {
			convert_camera_to_3d()
		}
		convert_target_to_3d(target, target_3d)
		if (active) {
			size_lerp.from = px_to_rad(size_lerp.from)
			size_lerp.to = px_to_rad(size_lerp.to)
		}
	} else if (state.camera.dimension == "2d") {
		if (pitch || yaw) {
			convert_camera_to_2d()
		}
		convert_target_to_2d(target_3d, target)
		if (active) {
			size_lerp.from = rad_to_px(size_lerp.from)
			size_lerp.to = rad_to_px(size_lerp.to)
		}
	}
}
/** @returns {void} */
function update_hud() {
	const { update_interval_ms, window_ms } = constants.hud
	const {
		sum_crit_ms,
		sum_hit_ms,
		sum_shoot_ms
	} = state.stats
	const { now_ms, start_ms } = state.timer
	state.hud.next_update_ms = now_ms + update_interval_ms
	const elapsed_s = min(now_ms - start_ms, window_ms) / 1_000
	const score = elapsed_s ? track_bits / elapsed_s : 0
	if (score) {
		run_count++
		run_sum += score
	}
	set_attr_if_changed(
		run_score_el,
		"value",
		`${round_to(score, 2)} / ${round_to(run_count ? run_sum / run_count : 0, 2)}`
	)
	set_attr_if_changed(
		accuracy_el,
		"value",
		`${(
			sum_shoot_ms
				? round_to(
					sum_hit_ms / sum_shoot_ms * 100,
					2
				)
				: 0
		)}%`
	)
	set_attr_if_changed(
		crit_rate_el,
		"value",
		`${(
			sum_hit_ms
				? round_to(
					sum_crit_ms / sum_hit_ms * 100,
					2
				)
				: 0
		)}%`
	)
	set_attr_if_changed(
		level_el,
		"value",
		`${round(staircase.level * 100)}`
	)
}
/** @type {GameMode} */
export default {
	check_stats,
	clear_score,
	dispose,
	init,
	on_frame,
	render,
	shoot,
	update_dimension,
	update_hud
}