...
function clear_best_score() {
	...
	send_toast(
		lang_text(
			{
				en: "Twitch score has been reset!",
				ko: "Twitch 점수를 초기화했습니다!"
			}
		),
		1_500
	)
...
function create_target_2d() {
	...
	const { height, width, x } = state.camera
...
function create_target_3d() {
	...
	const { fov, height, width, yaw } = state.camera
	const base_radius_rad = px_to_rad(base_radius)
	const random_x = random() - .5
	const r = base_radius_rad * (1 + abs(random_x) * 2 * (target_radius_mul_max - 1))
...
/** @returns {Target[]} */
function get_ordered_targets_2d() {
...
/** @returns {Target3D[]} */
function get_ordered_targets_3d() {
...
function render() {
	...
	if (dimension == "2d") {
		draw_grid()
		if (targets.length) {
			draw_aim_guides_2d(get_ordered_targets_2d())
		...
		if (targets_3d.length) {
			draw_aim_guides_3d(get_ordered_targets_3d())
