...
function on_frame() {
	...
	if (dimension == "2d") {
		...
		if (target.y >= range_px || target.y <= -range_px) {
			state.mode.v_tracking.move.direction = target.y >= range_px ? -1 : 1
			state.mode.v_tracking.move.direction_change_rate = 0
		...
		target.cy = target.y + target.cr - target.r
	} else {
		...
		if (target_3d.p >= range_rad || target_3d.p <= -range_rad) {
			state.mode.v_tracking.move.direction = target_3d.p >= range_rad ? -1 : 1
			state.mode.v_tracking.move.direction_change_rate = 0
			state.mode.v_tracking.next_change_move_ms = now_ms + constants.mode.v_tracking.move_change_interval_ms
		}
		if (speed_lerp.active) {
