...
const STORAGE_KEY = "v_tracking#best_score"
...
function clear_best_score() {
	...
	send_toast(
		lang_text(
			{
				en: "V-Tracking score has been reset!",
				ko: "V-Tracking 점수를 초기화했습니다!"
			}
		),
		1_500
	)
...
function init() {
	...
	if (state.camera.dimension == "2d") {
		...
		target_3d.cp = 0
		target_3d.cr = calc_core_radius(radius_rad)
		target_3d.cy = 0
		target_3d.p = 0
...
function on_frame() {
	...
	if (key_a || key_e || key_q || key_r || key_w || mb_left) {
		...
		if (dimension == "2d") {
			...
			if (target.y > range_px) {
				target.y = range_px
				if (move.direction > 0) {
					move.direction = -1
					move.direction_change_rate = 0
					move.speed = -abs(move.speed)
					speed_lerp.active = false
					next_change_move_ms = now_ms + move_change_interval_ms
				}
			} else if (target.y < -range_px) {
				target.y = -range_px
				if (move.direction < 0) {
					move.direction = 1
					move.direction_change_rate = 0
			...
			if (size_lerp.active) {
				...
				target_3d.cr = calc_core_radius(
					target_3d.r,
					px_to_rad(base_radius)
				)
			} else if (now_ms >= next_change_size_ms) {
				const index = random() * size_steps.length | 0
				size_lerp.active = true
			...
			if (target_3d.p > range_rad) {
				target_3d.p = range_rad
				if (move.direction > 0) {
					move.direction = -1
					move.direction_change_rate = 0
					move.speed = -abs(move.speed)
			...
			target_3d.cp = target_3d.p
			target_3d.cy = target_3d.y
		}
		shoot()
	}
}
...
/** @returns {void} */
function shoot() {
	const { impact_interval_ms } = constants.mode.v_tracking
