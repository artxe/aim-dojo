 ...
 * @returns {number}
 */
export function calc_camera_sens(fov_deg) {
	const { height, width } = state.camera
	if (state.game.sens == "bdo") {
		return calc_rad_per_px(
			convert_deg_across_aspect(
	...
	return calc_rad_per_px(fov_deg, width)
}
/** @returns {void} */
...
export function hit_test_2d(target, x, y) {
	...
	const dx2 = (target_x - x) ** 2
	hit_result.is_crit = dx2 + (cy - y) ** 2 <= cr * cr
	hit_result.is_hit = dx2 + (target_y - y) ** 2 <= r * r
	return hit_result
}
/**
 * @param {Target3D} target
 ...
 * @returns {boolean}
 */
export function is_target_visible_2d(target, x, y) {
	const { height, width } = state.camera
	const { x: target_x, y: target_y } = target
	return abs(target_x - x) <= width / 2
		&& abs(target_y - y) <= height / 2
}
/**
 * @param {Target3D} target
 * @param {number} yaw
 ...
 * @returns {boolean}
 */
export function is_target_visible_3d(target, yaw, pitch) {
	const { fov, vfov } = state.camera
	const { p, y } = target
	const cam_cos_pitch = cos(pitch)
	const cam_sin_pitch = sin(pitch)
	const body_cos_pitch = cos(p)
	const body_x = sin(y) * body_cos_pitch
	const body_y = sin(p)
	const body_z = -cos(y) * body_cos_pitch
	const forward = (body_x * sin(yaw) - body_z * cos(yaw)) * cam_cos_pitch
		+ body_y * cam_sin_pitch
	if (forward <= 0) {
		return false
	}
	const rise = body_y * cam_cos_pitch
		- (body_x * sin(yaw) - body_z * cos(yaw)) * cam_sin_pitch
	const side = body_x * cos(yaw) + body_z * sin(yaw)
	return abs(side) <= forward * tan(to_rad(fov) / 2)
		&& abs(rise) <= forward * tan(to_rad(vfov) / 2)
}
/**
 * @param {number} px
 * @returns {number}
