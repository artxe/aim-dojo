...
import {
	canvas_crosshair_el,
	canvas_el,
	crosshair_preview_el
} from "../controller/dom.js"
import { TAU, min, round } from "../math.js"
...
const context_crosshair_raw = canvas_crosshair_el.getContext("2d", { desynchronized: true })
const context_crosshair = /** @type {CanvasRenderingContext2D} */(context_crosshair_raw)/**/
...
export const grid_pattern = (() => {
	...
	off_context.save()
	off_context.lineWidth = minor_line_width
	off_context.strokeStyle = minor_stroke_style
	...
	off_context.stroke()
	off_context.lineWidth = major_line_width
	off_context.strokeStyle = major_stroke_style
	off_context.beginPath()
...
export function draw_aim_guides_2d(chain) {
	...
	context_2d.globalAlpha = stroke_alpha
 ...
 * @returns {void}
 */
export function draw_crosshair(x, y) {
	const {
		height: canvas_height,
		width: canvas_width
	} = canvas_crosshair_el
	const { height, width } = state.crosshair
	context_crosshair.clearRect(0, 0, canvas_width, canvas_height)
	context_crosshair.drawImage(
		crosshair_image,
		round(
			canvas_width / 2 + x - width / 2
		),
		round(
			canvas_height / 2 + y - height / 2
		)
	)
}
/** @returns {void} */
...
export function draw_impacts() {
	...
	while (index < count) {
		...
		if (progress <= 1) {
			...
			context_2d.beginPath()
			context_2d.arc(impact_x, impact_y, dot_radius, 0, TAU)
			context_2d.fill()
		}
...
export function resize_2d() {
	const { scale } = state.render
	const { x, y } = state.camera
	const ratio = scale / 100
	const width = state.camera.width = innerWidth * devicePixelRatio
	const height = state.camera.height = innerHeight * devicePixelRatio
	canvas_crosshair_el.height = height
	canvas_crosshair_el.width = width
	canvas_el.height = round(height * ratio)
	canvas_el.width = round(width * ratio)
	context_2d.setTransform(ratio, 0, 0, ratio, 0, 0)
	context_2d.save()
