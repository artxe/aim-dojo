...
import { PI, TAU, cos, round, sin, tan, to_rad } from "../math.js"
...
uniform mat4 u_view;
const float SEGMENTS = ${constants.render.circle_segments}.0;
...
const sky_sphere = (() => {
	const {
		sky_sphere_radius: r,
		sky_sphere_segments_h: seg_h,
		sky_sphere_segments_w: seg_w
	} = constants.grid
	/** @type {number[]} */
	const segments = []
...
const sky_sphere_major = (() => {
	const {
		sky_sphere_major_deg,
		sky_sphere_radius: r,
		sky_sphere_segments_h: seg_h,
		sky_sphere_segments_w: seg_w
	} = constants.grid
	const step = to_rad(sky_sphere_major_deg)
	/** @type {number[]} */
	const segments = []
	for (let p = step; p <= PI; p += step) {
		const cy = cos(p)
		const sy = sin(p)
		for (let x = 0; x < seg_w; x++) {
			const th1 = (x / seg_w) * TAU
...
export function draw_aim_guides_3d(chain) {
	...
	const { color_3d, line_width } = constants.guide
	...
	guide_info.count = n
	draw_stroke(color_3d, line_width, guide_info)
}
/**
 * @param {number} yaw
 * @param {number} pitch
 ...
 * @param {number} dist
 * @param {readonly [ number, number, number, number ]} color
 * @returns {void}
 */
function draw_disc(yaw, pitch, radius_rad, dist, color) {
	const { circle_segments } = constants.render
	const a = color[3]
...
export function draw_impacts_3d() {
	...
	for (let i = 0; i < count; i++) {
		...
		for (let k = 0; k < rings; k++) {
			...
			const a = alpha * (1 - fade_factor * ring_progress)
			if (a <= 0) {
				continue
			}
			const rgb = c == null
			...
			color[2] = rgb[2]
			color[3] = a
			draw_ring(y, p, ring_radius, d, color, line_width)
		}
	}
}
/**
...
export function prepare_3d_view() {
	...
	const { far, near } = constants.render
	frame_id++
	context_3d.clear(context_3d.COLOR_BUFFER_BIT)
	mat4.perspective(proj, vfov, width / height, near, far)
	mat4.view(
		view,
		yaw,
		pitch,
...
export function resize_3d() {
	...
	const buffer_height = round(height * ratio)
	const buffer_width = round(width * ratio)
	canvas.height = buffer_height
	canvas.width = buffer_width
	viewport_height = height
	viewport_width = width
	context_3d.viewport(0, 0, buffer_width, buffer_height)
}
{
