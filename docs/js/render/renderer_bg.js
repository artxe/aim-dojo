...
function create_bg_video_el(handle) {
	...
	video.preload = "auto"
	video.srcObject = /** @type {MediaProvider} */(handle)/**/
...
function get_bg_dpr() {
	...
	return (devicePixelRatio || 1) * scale / 100
}
/**
...
function post_bg_video_frame(metadata) {
	...
	const frame = new VIDEO_FRAME(
		bg_video_el,
		{
			timestamp: round(mediaTime * 1_000_000)
		}
	)
...
export function update_bg_video() {
	...
	const running = type == "default" && bg_video_visible
	if (type == "default") {
	...
	if (running && !bg_video_running) {
		restart_bg_video_frames()
	}
	bg_video_running = running
}
/** @returns {void} */
