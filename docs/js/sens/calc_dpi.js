...
export function calc_dpi_fn() {
	...
	const { height, width } = state.game
	if (fov == "hipfire") {
		return dpi * sens / calc_sens_fn(80, height, width)
	}
	const { zoom } = state.dpi_norm
	return dpi * sens * zoom / 100 / calc_sens_fn(80, height, width)
}
/** @returns {number} */
...
export function calc_dpi_ow() {
	...
	const { width, height } = state.game
	if (fov == "hipfire") {
		return dpi * sens / calc_sens_ow(103, height, width)
	}
	const { zoom } = state.dpi_norm
	let hfov_deg
	if (fov == "ashe") {
	...
	return dpi * sens * zoom / 100 / calc_sens_ow(hfov_deg, height, width)
}
/** @returns {number} */
...
export function calc_dpi_val() {
	...
	const base_hfov = 103
	if (fov == "hipfire") {
		return dpi * sens / calc_sens_val(base_hfov, height, width)
	}
	const { zoom } = state.dpi_norm
	let hfov_deg
	if (fov == "guardian") {
	...
	return dpi * sens * zoom / calc_sens_val(hfov_deg, height, width)
}