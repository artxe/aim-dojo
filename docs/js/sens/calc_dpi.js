...
export function calc_dpi_fn() {
	...
	if (fov == "hipfire") {
		return dpi * sens / calc_sens_fn_tpp(80, width)
	}
	const { zoom } = state.dpi_norm
	let hfov_deg
	if (fov == "ads") {
	...
	return dpi * sens * zoom / 100
		/ (compute_sens_rad(hfov_deg, width) / to_rad(.005_555 * hfov_deg / 80))
}
/** @returns {number} */
...
export function calc_dpi_pubg() {
	...
	if (fov == "tpp") {
		...
		return dpi_sens_rad
			/ compute_sens_rad_tpp(base_fov, width)
	}
	let hfov_deg
	if (fov == "x1") {
		hfov_deg = base_fov
