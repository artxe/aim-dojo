...
export function calc_sens_fn(hfov_deg, width, height) {
	return calc_rad_per_px(hfov_deg, width, height)
		/ to_rad(
...
export function calc_sens_mc(vfov_deg, width, height) {
	...
	const rad_per_count = calc_rad_per_px(hfov_deg, width, height)
	return (
 ...
 * @returns {number}
 */
export function calc_sens_ow(hfov_deg, width, height) {
	return calc_rad_per_px(hfov_deg, width, height)
		/ to_rad(.006_6)
}
/**
...
export function calc_sens_sa(height) {
	return (calc_rad_per_px(85, height * 4 / 3, height) - .000_15)
		/ .000_03
}
/**
 * @param {number} hfov_deg
 ...
 * @returns {number}
 */
export function calc_rad_per_px(fov_deg, width, height) {
	const hfov = to_rad(fov_deg)
	const vfov = 2 * atan(
		tan(hfov / 2) * (height / width)
	)
	const solid_angle = 4 * asin(sin(hfov / 2) * sin(vfov / 2))
	const total_pixels = width * height
	return sqrt(solid_angle / total_pixels)
}