...
const [ crit_sound, hit_sound, miss_sound ] = await Promise.all(
	[
		(() => {
		...
		(() => {
			...
			const off = new OfflineAudioContext(1, ceil(sr * duration), sr)
			const t0 = 0
			const noise_frames = round(sr * .06)
			const noise_buf = off.createBuffer(1, noise_frames, sr)
			const ch = noise_buf.getChannelData(0)
			for (let i = 0; i < noise_frames; i++) {
				ch[i] = (random() * 2 - 1) * (1 - i / noise_frames)
			}
			const noise = off.createBufferSource()
			noise.buffer = noise_buf
			...
			osc.frequency.exponentialRampToValueAtTime(60, t0 + .1)
			const osc_gain = off.createGain()
			osc_gain.gain.value = .3
			const env = off.createGain()
			env.gain.setValueAtTime(0, t0)
			env.gain.linearRampToValueAtTime(1, t0 + .006)
			env.gain.exponentialRampToValueAtTime(.001, t0 + .1)
			noise.connect(lowpass)
			lowpass.connect(env)
			osc.connect(osc_gain)
			osc_gain.connect(env)
 ...
 * @returns {void}
 */
function update_sfx_volume(t = get_audio_time()) {
