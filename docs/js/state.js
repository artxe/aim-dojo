...
export default {
	...
	game: {
		dpi_scale: lol_sens_to_dpi_scale(game_lol_sens),
		height: game_height,
		lol_sens: game_lol_sens,
		/** @type {GameModeName?} */
		mode: null,
		/** @type {number} */
		raf_id: 0,
	...
	impact: {
		px_size: 20,
		rad_size: 2 * tan(103 * PI / 360) / game_width * 20
	},
	/**
	...
	mode: {
		...
		full_tracking: {
			best_score: Number(
				localStorage.getItem("full_tracking.best_score") || 0
			),
			move: {
				/** @type {1|-1} */
				direction: 1,
		...
		timing: {
			best_score: Number(
				localStorage.getItem("timing.best_score") || 0
			),
			next_hide_ms: 0,
			next_show_ms: 0,
			peak_score: 0,
