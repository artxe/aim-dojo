...
**A worker is for work that would hitch a frame — not for "this looks like computation".** The ENTIRE 73-cell sens card measures **~0.18 ms** (~100 % of it `calc_sens_r6_file`'s grid scan; the other ~50 `calc_sens_*` calls are below noise), so it runs inline in `controller/game_sens.js`. It was `calc_worker.js:update_game_sens` shipping a positional number tuple four files had to edit in lockstep — **don't put it back**. The bar `check_writing_stats` clears and sens never did: it rasterizes ~1546 × 1200 (15 sentences × `grid.size` 80), `getImageData`s ~7 MB and walks 1.86 M alpha bytes, **every 50 ms** while the mode runs.
...
- `bg_worker.js` — default-background pipeline: fetches/fragments `bg.mp4` CROSS-ORIGIN from the `workers.dev` Worker (`cloudflare/CLAUDE.md` — CORS, caching, limits), owns the worker-side `MediaSource` + WebGL2 draw into a transferred `OffscreenCanvas`. Imports ONLY `../constants.js` + `../math.js`
...
|---|---|---|---|
| `check_writing_stats` | `drop_count, line_width, segments` | `[fn, count_hit, count_shoot]` | `game_mode/writing.js`; if still in writing mode, writes `state.stats.count_hit/count_shoot` (bypasses the `shoots` queue) |
...
## Non-obvious
- **`check_writing_stats` is a DELTA protocol, and the worker owns the stroke history.** The message carries only the strokes added since the last tick — a flat `Float64Array` of `ex,ey,sx,sy` quads, TRANSFERRED — plus `drop_count`, how many strokes fell out of the 30 s window in the same span. The worker keeps its own `Float64Array` ring (`lines`/`lines_head`/`lines_tail`, `LINE_STRIDE` 4, doubling + `copyWithin` compaction) and rebuilds the raster from it. **Never send the whole stroke list again**: at a 30 s @240 fps window that is 7 200 objects, and structured-cloning them measured **3.07 ms per post × 20 Hz = 61 ms/s of MAIN-thread time** — the same payload as a `Float64Array` measures 0.043 ms, and transferred it is free. `set_writing_text` resets the worker ring, so a new run cannot inherit the last one's strokes
- `check_writing_stats` reuses a module-scope OffscreenCanvas (`off`/`off_context`) sized ONCE by `set_writing_text` and `clearRect`'d per call — do NOT go back to re-assigning `off.width`/`off.height` per call, which reallocated the ~7 MB bitmap 20×/s (it also silently reset the 2D context state, which is why `lineWidth` is set every call). The pixel loop walks alpha bytes (`alpha_i = 3; alpha_i += 4`) counting `line_image_data[alpha_i]` against `text_data[alpha_i]`
- `post_writing_worker_message()`/`post_bg_worker_message()` are thin one-liners over `postMessage` with two JSDoc `@overload`s for the transfer-list vs options call shapes. They USED to have two `Array.isArray` branches with byte-identical bodies; that was dead weight, not overload resolution
...
- BG holds parsed `bg.mp4` fragments in memory but appends to the `SourceBuffer` only around current playback (`constants.bg.append_ahead_s`, evicting `evict_behind_s` behind). Do not restore eager append-all; large replacement videos trip the SourceBuffer quota
