...
`
const BG_VIDEO_URL = "https://aim-dojo.artxe.workers.dev/bg.mp4"
const MIME = "video/mp4; codecs=\"avc1.64002A,mp4a.40.2\""
const POST_BG_REPLY = /** @type {(message: BgWorkerReply, transfer?: Transferable[]) => void} */(postMessage)/**/
...
function append_next_bg_item() {
	...
	if (
		...
		bg_append_queue.copyWithin(0, bg_append_queue_head)
		bg_append_queue.length -= bg_append_queue_head
...
function draw_bg_video(frame) {
	...
	const canvas_height = ceil(height * dpr)
	const canvas_width = ceil(width * dpr)
...
function evict_bg_behind() {
	...
	const cutoff = bg_current_time - constants.bg.evict_behind_s
...
async function get_cached_bg(version) {
	...
	return new Promise(
		(resolve, reject) => {
			const request
			/** @type {IDBRequest<Blob | undefined>} */= (db.transaction(BG_CACHE_STORE, "readonly")
...
function schedule_bg_append() {
	...
	if (buffered_end - t >= constants.bg.append_ahead_s) {
		evict_bg_behind()
		return
	}
...
{
	/** @param {MessageEvent<BgWorkerRequest>} event */
