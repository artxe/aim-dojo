...
async function load_bg_fragments() {
	...
	let response = await cache.match(BG_VIDEO_URL)
	if (!response) {
		response = await fetch(BG_VIDEO_URL)
		cache.put(BG_VIDEO_URL, response.clone())
	}
	const reader = /** @type {ReadableStream<Uint8Array<ArrayBuffer>>} */(response.body)/**/.getReader()
	while (true) {
...
function parse_bg_fragments(done) {
	...
	if (done) {
		if (bg_parse_pending.length) {
			append_bg_fragment(
				slice_bg_bytes(0, bg_parse_pending.length)
			)
