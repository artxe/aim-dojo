...
function check_writing_stats(
	...
	for (let i = 0; i < line_count; i++) {
		const { ex, ey, sx, sy } = /** @type {Line} */(lines.at(i))/**/
	...
	off_context.stroke()
	off_context.restore()
	const line_image_data = off_context.getImageData(0, 0, width, height).data
	let count_hit = 0
	let count_shoot = 0
	const pixel_data_len = text_data.length
	for (let alpha_i = 3; alpha_i < pixel_data_len; alpha_i += 4) {
		if (line_image_data[alpha_i]) {
			count_shoot++
			if (text_data[alpha_i]) {
				count_hit++
			}
		}
	}
 ...
 * @returns {void}
 */
function update_game_sens(width, height, dpi_scale) {
	const calc_height = height / dpi_scale
	const calc_width = width / dpi_scale
	const al_hipfire = round_to(
		calc_sens_al(70 * 1.55, calc_width, calc_height),
		2
	)
	const al_x1 = round_to(
		calc_sens_al(60 * 1.55, calc_width, calc_height) / al_hipfire,
		2
	)
	...
	const cs2_hipfire = round_to(
		calc_sens_cs2(90, calc_width, calc_height),
		2
	)
	const cs2_45 = round_to(
		calc_sens_cs2(45, calc_width, calc_height) / cs2_hipfire,
		2
	)
	const cs2_40 = round_to(
		calc_sens_cs2(40, calc_width, calc_height) / cs2_hipfire,
		2
	)
	const cs2_15 = round_to(
		calc_sens_cs2(15, calc_width, calc_height) / cs2_hipfire,
		2
	)
	const cs2_10 = round_to(
		calc_sens_cs2(10, calc_width, calc_height) / cs2_hipfire,
		2
	)
	const fn_hipfire = round_to(calc_sens_fn(80, calc_width), 1)
	const fn_targeting = 100
	const fn_scope = 100
	const mc_hipfire = round_to(
		calc_sens_mc(110, calc_width, calc_height),
		16
	)
	const ow_hipfire = round_to(calc_sens_ow(103, calc_width), 2)
	...
	const pubg_hipfire = calc_sens_pubg(80, calc_width)
	const pubg_ads = calc_sens_pubg(80 / 1.5, calc_width)
	const pubg_x2 = calc_sens_pubg(80 / 2, calc_width)
	const pubg_x3 = calc_sens_pubg(80 / 3, calc_width)
	const pubg_x4 = calc_sens_pubg(80 / 4 - 1, calc_width)
	const pubg_x6 = calc_sens_pubg(80 / 6, calc_width)
	const pubg_x8 = calc_sens_pubg(80 / 8, calc_width)
	const pubg_x15 = calc_sens_pubg(80 / 15, calc_width)
	const r6_hipfire = round(
		calc_sens_r6(90, calc_width, calc_height)
	)
	const r6_x1 = round(
		calc_sens_r6(81, calc_width, calc_height) / r6_hipfire * 50
	)
	const r6_x2_5 = round(
		calc_sens_r6(
