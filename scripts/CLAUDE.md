...
- Hardcoded constants (NOT env-configurable): `HOST` (0.0.0.0), `PORT` (3000), `LIMIT_MB_S` (10). Throttle works by pausing the read stream and `setTimeout`-resuming to hit `sent_bytes / BYTES_PER_MS`.
- Sends `cache-control: no-cache, no-store, must-revalidate` (forces re-fetch each load), supports Range (`206`)/`If-None-Match`/`If-Modified-Since` (`304`), and guards path traversal (resolved path must stay under `docs/`).
