...
- No bundler ⇒ the browser discovers each module only after parsing its importer. Boot graph = **34 modules / 272 KB / 17 levels deep**, so without preload the floor is ~17 sequential round trips; the generated block hands over every URL up front
